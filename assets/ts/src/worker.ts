...
function gen_round_keys(key, K) {
  ...
  for(i = 0; i < 56; i++)
    parity_drop[i] = key[ parity_drop_table[i] ];
...
function crypt3(_pwd, _salt, is_wasm, wasm = {}) {
  ...
  let L = Array(32);
  let R = Array(32);
  let K = Array(16).fill(0).map(_=>Array(48).fill(0));
  let data = Array(64);
  let digest = Array(13);
...
async function main_wasm() {
  ...
  for(let i = 0; i < run_n; i++) {
    ...
    console.log(i);
    console.log(read_mem(mem, i) );
    console.log( mem.slice(20000 + i*13, 20000 + i*13+13) );
  }
